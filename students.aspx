<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="students.aspx.cs" Inherits="CourseTimetable.students" %>
...
<body style="font-size: x-small">
    ...
    <div class="auto-style1">
     <div class="jumbotron">
        <h1 style="background-color: #66CCFF">Student Records Page</h1>
        <p class="lead">You can view student records below, you may update, delete or add new records.</p>
        
    </div>
        <strong style="font-size: x-small"><br />
        <asp:ListView ID="ListView1" runat="server" DataKeyNames="Student_ID" DataSourceID="SqlDataSource1" InsertItemPosition="LastItem" OnSelectedIndexChanged="ListView1_SelectedIndexChanged" style="font-size: small">
        ...
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConflictDetection="CompareAllValues" ConnectionString="<%$ ConnectionStrings:ct_ds_courses %>" DeleteCommand="DELETE FROM [Student_Table] WHERE [Student_ID] = @original_Student_ID AND [First_name] = @original_First_name AND (([initial] = @original_initial) OR ([initial] IS NULL AND @original_initial IS NULL)) AND [Surname] = @original_Surname AND [Address] = @original_Address AND [Birthday] = @original_Birthday AND [Gender] = @original_Gender AND [Grade_ID] = @original_Grade_ID" InsertCommand="INSERT INTO [Student_Table] ([Student_ID], [First_name], [initial], [Surname], [Address], [Birthday], [Gender], [Grade_ID]) VALUES (@Student_ID, @First_name, @initial, @Surname, @Address, @Birthday, @Gender, @Grade_ID)" OldValuesParameterFormatString="original_{0}" SelectCommand="SELECT [Student_ID], [First_name], [initial], [Surname], [Address], [Birthday], [Gender], [Grade_ID] FROM [Student_Table]" UpdateCommand="UPDATE [Student_Table] SET [First_name] = @First_name, [initial] = @initial, [Surname] = @Surname, [Address] = @Address, [Birthday] = @Birthday, [Gender] = @Gender, [Grade_ID] = @Grade_ID WHERE [Student_ID] = @original_Student_ID AND [First_name] = @original_First_name AND (([initial] = @original_initial) OR ([initial] IS NULL AND @original_initial IS NULL)) AND [Surname] = @original_Surname AND [Address] = @original_Address AND [Birthday] = @original_Birthday AND [Gender] = @original_Gender AND [Grade_ID] = @original_Grade_ID">
            <DeleteParameters>
